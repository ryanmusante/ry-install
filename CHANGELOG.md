...
v4.0.0  2026-04-18
- Major version marker: this release consolidates the v3.5x audit cycle into a new stable baseline. No functional regressions versus the prior internal v3.51.15 build; all changes below are the audit follow-through that would have shipped as v3.52.0, re-labelled to v4.0.0 to reflect the accumulated embedded-config drift (KERNEL_PARAMS count 12 → 15, SYSCTL_VALUES 21 → 19, PKGS_ADD 15 → 14, EXPECTED_SERVICES 3 → 4) and a new runtime dependency (`nftables`).
- `amd_pstate=active` added to KERNEL_PARAMS (HIGH): forces the amd_pstate_epp driver on Zen 5 with lowest-latency P-state transitions; preferred-core (ITMT routing to highest-Fmax cores) is default-on when supported and has no explicit flag (kernel docs only define `amd_prefcore=disable`). Complements the existing static `EPP=performance` via `cpupower-epp.service`.
- RADV environment split (HIGH): `RADV_EXPERIMENTAL=transfer_queue` in ENV_VARS replaced by `RADV_EXPERIMENTAL=transfer_queue,hic` plus a new `RADV_PERFTEST=sam,nircache`. Mesa 25.x/26.x marks `transfer_queue` and `hic` as deprecated under RADV_PERFTEST and routes them through RADV_EXPERIMENTAL; `sam` and `nircache` reduce upload stalls and pipeline compile time on RDNA3.5 UMA.
- `nftables` stateful firewall baseline restored (HIGH): package added to PKGS_ADD; `nftables.service` added to the `sys_enable` batch and to EXPECTED_SERVICES so regressions are caught by the existing verify phase. Uses the package-default `/etc/nftables.conf` (loopback accept, ESTABLISHED/RELATED accept, ICMP allowances, default drop inbound) — no hand-rolled ruleset. Closes the host firewall gap left by prior `ufw` removal.
- `clocksource=tsc` → `tsc=reliable` in KERNEL_PARAMS (MED): bypasses the TSC watchdog (common "marked unstable" source on Zen 5) without force-selecting the clocksource. Zen 5 TSC is invariant and auto-selected by the kernel.
- `nvme_core.default_ps_max_latency_us=0` removed from KERNEL_PARAMS (MED): blocked all APST states and raised NVMe idle power on a compact mini-PC; only needed for known-faulty drives. The NVMe APST verify block at line 3904 flipped to a regression detector — it now fails if the sysfs value is 0 (indicating the cmdline param was re-added) and records the live module-default value otherwise.
- `fs.inotify.max_user_watches=524288` removed from SYSCTL_VALUES (MED): kernel 5.11+ auto-scales to ~1% of RAM (1,048,576 on a 128 GiB host); the explicit value was halving the default.
- `net.ipv4.tcp_notsent_lowat=131072` added to SYSCTL_VALUES (MED): limits unsent data queued in the TCP send buffer, reducing head-of-line latency for interactive game and voice flows under the BBR+fq pair already configured by `net.core.default_qdisc=fq` and `net.ipv4.tcp_congestion_control=bbr`. 128 KiB is the Cloudflare/Dropbox production value.
- `systemd-coredump.socket` added to MASK (MED): `Storage=none` in `/etc/systemd/coredump.conf.d/99-cachyos-coredump.conf` already suppresses coredump storage, but the kernel still invokes systemd-coredump via the socket per crash. Masking the socket eliminates that spawn-and-discard overhead on Wine/Proton crashes.
- `RESOLVED_MDNS` default flipped `no` → `resolve` (MED): `no` disables both the mDNS responder AND the resolver, breaking `.local` hostname resolution under COSMIC (SMB/NAS, AirPrint, KDE Connect, Chromecast). `resolve` keeps resolution working without advertising this host on the network. The deployed file at `/etc/systemd/resolved.conf.d/99-cachyos-resolved.conf` is generated from the variable.
- `loglevel=3`, `rd.udev.log_level=3`, `rd.systemd.show_status=auto` added to KERNEL_PARAMS (LOW): cleaner boot output alongside existing `quiet`; `show_status=auto` still prints on errors. `LINUX_FALLBACK_OPTIONS="quiet"` at line 1079 is separately managed and unchanged.
- `net.core.somaxconn=8192` removed from SYSCTL_VALUES (LOW): kernel 5.4 default is 4096; the bump was cosmetic for a desktop workload.
- `kernel.unprivileged_bpf_disabled=1` removed from SYSCTL_VALUES (LOW): CachyOS kernel has `CONFIG_BPF_UNPRIV_DEFAULT_OFF=y`, making the sysctl a no-op.
- drirc developer comment at line 1170 corrected (LOW): `Requires Mesa ≥25.0` → `Requires Mesa ≥22.3`. `radv_enable_unified_heap_on_apu` has been available since Mesa 22.3 (MR !18884, September 2022); the prior comment overstated the version requirement by ~3 major releases. Unicode `≥` preserved; the `/etc/drirc` XML payload itself is unchanged.
- MKINITCPIO_MODULES trimmed `amdgpu nvme` → `amdgpu` (LOW): the `block` hook plus `autodetect` already pull the nvme module; explicit listing was redundant.
- `irqbalance.service` removed from MASK and its dedicated `_ry_verify_runtime` stanza also dropped (LOW): irqbalance is not installed by default on CachyOS (cachyos-settings deps do not pull it in); masking a non-installed unit was defensive cruft. Trust-the-user model — if irqbalance is later installed manually, it is assumed to be intentional.
- `vulkan-radeon` and `lib32-vulkan-radeon` removed from PKGS_ADD (LOW): CachyOS chwd auto-installs them on AMD GPU hosts via `profiles/pci/graphic_drivers/profiles.toml` (packages = `mesa lib32-mesa vulkan-radeon lib32-vulkan-radeon xf86-video-amdgpu gst-plugin-va linux-firmware-amdgpu`). Listing them in PKGS_ADD created dual-ownership ambiguity. `libva-mesa-driver` and `lib32-libva-mesa-driver` are NOT in the chwd profile (gst-plugin-va is the GStreamer wrapper, not the VA-API driver) and remain in PKGS_ADD. `EXPECTED_VULKAN_PKGS` runtime check retained — chwd-installed packages still satisfy it.
- README "Deprecated flags — DO NOT re-introduce" subsection added under Environment Variables (LOW): lists `DXVK_ASYNC`, `DXVK_FRAME_RATE`, `WINE_FULLSCREEN_FSR` (all removed upstream) plus `VKD3D_FRAME_RATE` retained as contrast. Prescriptive documentation to prevent re-introduction during future contributions.
