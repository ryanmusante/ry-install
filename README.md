...
**Version 7.187.0** · [Changelog](CHANGELOG.md)
...
`ry-install.fish` gates the tools below at preflight. `ry-verify.fish` runs no dependency phase — it still refuses to start without GNU `id`, `find`, `stat`, `date`, `mktemp`, and `mv -T`, and guards the rest at their call sites; `curl`, `mkinitcpio`, and `sdboot-manage` are install-only.
...
| `4` | boot-critical (`ry-install.fish`) — boot cascade or post-rebuild sanity failed; **do not reboot**, resolve first |
| `5` | lock (`ry-install.fish`) — another instance holds the lock; ambiguous pidfiles fail closed |
| `10` | drift (`ry-verify.fish`) — `--check` found drift from the managed baseline |
