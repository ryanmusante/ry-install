...
-------------------
  ...
  * atomic-write: `_atomic_write_file`, `_mkinitcpio_revert`, `_fstab_atomic_replace`, `_if_write_wipe_marker`, `_acquire_lock_fresh` switched from `mv --` / `mv -f --` to `mv -T --` / `mv -Tf --`. Refuses target-as-directory; without `-T`, `mv` silently relocates the source *into* the target if the destination ever became a directory, leaving the live config tree intact and the deploy reporting success.
  * lock: `_reclaim_stale_lock` liveness probe switched from `kill -0 -- $old_pid` to `test -d /proc/$old_pid`. `kill -0` returns rc=1 on both ESRCH (no such process) and EPERM (process owned by another user) — the latter caused the fish-ownership branch to be skipped against another user's running ry-install. /proc lookup is owner-independent.
  * pacman: `_csp_remove_pkgs` and `_ip_pacman_invoke` re-test `/var/lib/pacman/db.lck` after a failing pacman invocation. Previous behaviour conflated db-lock-during-run with generic batch failure; now emits `Pacman database became locked during removal/install — aborting` (with `PKG_REMOVE_BATCH_FAIL_DBLOCK` JSONL marker) when the lock appeared mid-run.
  * aur: `_install_aur_packages` skips per-package retry when `count $AUR_PKGS` ≤ 1. With a single-element list (the current `mt76-mt7925-dkms`) the per-package loop re-ran the same paru invocation that just failed, doubling the error output for no diagnostic value.
  * check: `_check_phase_files` now consumes the `_installed_bytes` rc directly (0=ok, 1=read fail → drift, 2=sudo lapse → `EXIT_PREFLIGHT`). Previous `test -z "$actual"` conflated read-failure and sudo-lapse for system destinations, returning `EXIT_PREFLIGHT` for the missing-file case which is correctly drift. Generator-empty branch hoisted above the read so generator failure short-circuits before the read attempt.
  * finalize: `_install_finalize` gates `_if_write_wipe_marker` on `INSTALL_HAD_ERRORS=false` in addition to `SDBOOT_REMOVE_EXISTING=yes`. Prevents the marker hash from advancing when the deploy ran with errors in earlier phases — keeps the auto-ack fast path honest on subsequent runs.
  * style: comment-pass — six multi-line comment blocks collapsed to single-line form (`_RY_BOOT_TAINTED` rationale, two DECSTBM cursor-position annotations, per-installed-kernel ntsync advisory, enable-ok-vs-start-fail comment, log-rotation pipestatus rationale). Log-rotation comment also corrected: only `string split0` returns rc=1 on empty input; `sort -zn` returns 0. Safe-lint markers (`# FISH-LINT-DIRECTIVE`, `# lint:ignore`) and the 2-line script header preserved verbatim.
  * docs: README `Install Flow` row reworded `mask 10 power-management units` → `mask 11 desktop/power units (5 sleep targets + 6 service/socket masks; lvm2-monitor auto-skipped when LVM detected)`; matches `MASK` count and the `_mask_list_effective` LVM gate.
  * release: patch version bump `4.6.8` → `4.6.9` (script header line 2 + `set -g VERSION` + README badge).

v4.6.8 - 2026-05-08
-------------------

  * boot: critical fix — `_pbs_entry_has_valid_kernel` treated a leading `/` in BLS Type #1 `linux=` fields as a system-absolute path, then containment-checked against ESP. Per [BLS](https://uapi-group.org/specifications/specs/boot_loader_specification/), `linux=` is anchored on `$BOOT` regardless of leading slash — the slash is partition-relative. Old branch `string match -q '/*' …; and set linux_check "$linux_line"; or set linux_check "$esp/$linux_line"` replaced with unconditional leading-slash strip + rejoin: `set linux_rel (string replace -r '^/+' '' -- "$linux_line"); set linux_canon (realpath -m -- "$boot/$linux_rel")`. Containment check retained as `..`-traversal defense; warn message reworded `escapes ESP boundary` → `escapes \$BOOT boundary`. Reproduced in v4.6.7 install log: `_preflight_boot_sanity` rejected both `linux-cachyos.conf` + `linux-cachyos-lts.conf` after `mkinitcpio -P` and `sdboot-manage gen` had succeeded, bailed with `EXIT_BOOT_CRIT` (4) and skipped `[6/6] Finalize` despite `bootctl status` reporting both entries valid.
  * boot: new `_resolve_boot_path` helper — mirrors `_resolve_esp` but uses `bootctl -x` (`--print-boot-path`); falls back to `_resolve_esp` when bootctl is missing or returns empty. Cached via `_RY_BOOT_PATH` (erased in `_ry_namespace_cleanup`). `_preflight_boot_sanity` and its three sub-helpers (`_pbs_check_kernels`, `_pbs_check_initrds`, `_pbs_check_entries`) now operate on `$BOOT`; parameter renamed `esp` → `boot`, error messages updated. Where `$BOOT` and ESP collapse (no separate XBOOTLDR partition) behaviour is unchanged. Other `_resolve_esp` call sites in `_install_rebuild_boot` and `_if_write_wipe_marker` left as-is — those drive ESP-specific operations.
  * release: patch version bump `4.6.7` → `4.6.8` (script header line 2 + `set -g VERSION` + README badge).
