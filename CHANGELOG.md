...
- Tagged as v3.48.21
- Removed 4 stale narrative comments that still referenced the `--lint` mode removed in v3.48.20. Lines 910 (`_load_profile` UUID hard-fail list), 1607 (`_progress_done` runtime assertion), 5604 (`_ry_do_test_all` sudo-cache rationale), 5922 (top-level dispatch lock-acquire comment). The inline `# lint:ignore` markers in embedded sh/awk/PCRE blocks are intentionally left in place — they are inert comments and removing them is a separate cosmetic pass per the v3.48.20 changelog note.
- `_run`: removed dead `/dev/null` initialization of `stderr_tmp` / `stdout_tmp`. The variables were initialized to `/dev/null`, immediately overwritten in the mktemp-success branch, and the mktemp-failure branch returns 1 before any consumer could observe the default. Declarations are now empty (`set -l stderr_tmp` / `set -l stdout_tmp`) and the two `test "$X" != /dev/null; and test -s "$X"` guards at lines 1673/1687 are collapsed to `test -s "$X"` — the `!= /dev/null` half was unreachable under both the old and new init because the only code path that reaches those guards has both variables set to real tmpfile paths inside `$_run_dir`. Net: -6 tokens, 0 semantic change, one less dead-code smell on next audit.
- No line count change (still 6001); this is a pure-polish pass.
