...
-------
  ...
  - changelog: condense the 7.139.0 - 7.183.0 block; 55 bullets fold
    to 34
...
-------
  ...
  - preflight: NM_WIFI_POWERSAVE now takes 0-3; any non-negative
    integer passed, and NetworkManager accepts only those four
...
-----------------
  ...
  - boot: COMPRESSION_OPTIONS -1 -> -3, drop the redundant -T0;
  ...
  - env: PROTON_FSR4_UPGRADE=1 -> FSR4_WATERMARK=1 -> at 7.181.0
    PROTON_FSR4_INDICATOR=1; FSR4_WATERMARK is inert under Proton-CachyOS
  ...
  - install-file: /boot post-hook keys on the exact path, not /boot/*; an
    unmatched post-hook WARNs, not log-only
  ...
  - verify: compare cmdline under the file's own UUID when findmnt fails;
    report .ry.bak/.ry.orig copies, fail on an empty one
  - verify: live ext4 mount options for fstab-listed rows, paths decoded;
    assert MangoHud layout and the empty mkinitcpio BINARIES and FILES
  ...
  - cleanup: 7.181.0 - 7.182.2 erase and sweep only names and globs each
    script can set; 31 verify-side and 3 install-side names never existed
  - check: record unmanaged 60-ry-* drop-ins before the sudo gate;
    mode-only drift sets drift, matching verify
  ...
  - summary: aligned columns replace the box-drawn table; 4 fns collapse
    to 1
  - split: 7.177.0 moves verify and check to ry-verify.fish; ry-install
    keeps install and install-file, shared functions duplicated verbatim
  ...
  - counts: SYSCTL_VALUES 9 -> 8, PKGS_ADD 16 -> 17; 7.182.0 splits the
    exit constants, ry-verify tripwires 21 -> 19, globs 6 -> 2 at 7.182.2
