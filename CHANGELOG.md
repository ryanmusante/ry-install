...
-------------------
  ...
  * boot: critical fix — `_pbs_entry_has_valid_kernel` interpreted a leading `/` in BLS Type #1 `linux=` fields as a system-absolute path. Per the [Boot Loader Specification](https://uapi-group.org/specifications/specs/boot_loader_specification/), `linux=` is **always** anchored on `$BOOT` (XBOOTLDR if separate, else ESP) — the leading slash is partition-relative, not filesystem-absolute. Old code: `string match -q -- '/*' "$linux_line"; and set linux_check "$linux_line"; or set linux_check "$esp/$linux_line"` → `realpath -m /vmlinuz-linux-cachyos-lts` → `/vmlinuz-linux-cachyos-lts` → fails the ESP-containment check → emits `Loader entry escapes ESP boundary` warning → entry rejected → `_pbs_check_entries` finds no valid entry → `No boot entry references a valid kernel image` err → `_preflight_boot_sanity` returns 1 → `_install_rebuild_boot` bails with `EXIT_BOOT_CRIT` (4) → `[6/6] Finalize` skipped. Reproduced in v4.6.7 install log: 6 fail / 3 warn / exit_code=4 on a system where `bootctl status` confirmed both entries valid and `test -f /boot/vmlinuz-linux-cachyos-lts` returned 0. New code: `set linux_rel (string replace -r '^/+' '' -- "$linux_line"); set linux_canon (command realpath -m -- "$boot/$linux_rel" …)` — strips any leading slashes unconditionally and rejoins under `$boot`. Containment check retained as `..`-traversal defense; warn message updated to `Loader entry escapes \$BOOT boundary` to reflect what's actually being enforced. Verified against the real entry shape `linux /vmlinuz-linux-cachyos-lts` (passes) plus traversal (`/../../../etc/passwd` → caught + rejected), no-leading-slash BLS form (passes), missing kernel file (rejected silently, no false escape warn), missing `linux=` line (rejected silently), and triple-slash defensive strip (passes).
  * boot: new `_resolve_boot_path` helper — mirrors `_resolve_esp` but uses `bootctl -x` (`--print-boot-path`) instead of `bootctl -p` (`--print-esp-path`). Falls back to `_resolve_esp` when `bootctl` is missing or returns empty. Cached via new `_RY_BOOT_PATH` global (erased in `_ry_namespace_cleanup`). `_preflight_boot_sanity` and its three sub-helpers (`_pbs_check_kernels`, `_pbs_check_initrds`, `_pbs_check_entries`) now operate on `$BOOT` instead of `$esp`; parameter renamed `esp` → `boot` and all error messages updated. On systems with no separate XBOOTLDR partition (the common case, including this profile) `$BOOT` and ESP collapse to the same path — behaviour unchanged. On systems with a separate XBOOTLDR partition (`bootctl -p` returns `/efi`, `bootctl -x` returns `/boot`), kernel/initramfs/loader-entry enumeration now correctly targets `/boot` per BLS — previously would have errored with `No vmlinuz found in /efi/`. Other `_resolve_esp` call sites (3 in `_install_rebuild_boot`/related at lines ~4420/4426/4436) unchanged — those use ESP for ESP-specific operations (sdboot-manage invocation, `EFI/` subtree).
