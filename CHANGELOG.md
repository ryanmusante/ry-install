...
-------
  ...
  - readme: backups and uninstall step 3 now state that a .ry.bak copy exists
    only where a run actually rewrote that file
...
-----------------
  ...
  - network: 7.200.0 disables NetworkManager connectivity checking in the
    drop-in
  - env: 7.195.0 drops PROTON_FSR4_INDICATOR=1
  - configuration: 7.195.0 MangoHud ships cpu_stats enabled, cpu_temp
    stays off
  ...
  - services: 7.198.0 dmemcg-booster-system.service joins EXPECTED_SERVICES
    as a package-managed unit; 7.200.0 removes it again
  ...
  - split: 7.195.1 - 7.195.2 lockstep bumps, comments and descriptions only;
    7.200.1 a verify-side check-logging bump, ry-install.fish unchanged
  - counts: 7.195.0 ENV_VARS 10 -> 9, SYSCTL_VALUES 8 -> 9; 7.198.0 PKGS_ADD
    17 -> 19, EXPECTED_SERVICES 5 -> 6; 7.199.0 KERNEL_PARAMS 14 -> 15
  - counts: 7.200.0 PKGS_ADD 19 -> 17, EXPECTED_SERVICES 6 -> 5 and the
    package-managed list 2 -> 1; every tripwire follows
