...
7.19.2  2026-06-02
- cleanup: drop dead SERVICE_DESTINATIONS subsystem + remnants; _RY_POST_HOOKS 17 -> 16.
- comment: preflight banner KERNEL VERSION -> DEPS + NETWORK + DISK + SYSTEMD.
- docs: regdom falls back to /etc/iw-regdomain when iw absent; NetworkManager-dispatcher opportunistic.

7.19.1  2026-06-02
- cmdline: iommu=pt -> amd_iommu=off; KERNEL_PARAMS stays 13.
- docs: install-flow file count 16 -> 15; --country=XX honored in all modes.
- format: run-summary evidence column 30 -> 50 cols.

7.19.0  2026-06-02
- pkgs: drop iw, rtkit (CachyOS defaults); 16 -> 14.
- files: drop /etc/modules-load.d/i2c-dev.conf (ddcutil ships it); 16 -> 15.
- change: wireless regdom -> /etc/iw-regdomain (COUNTRY=).
- fix: ntsync autoload verify scans candidate confs (--verify only).

7.18.0  2026-06-01
- remove: drop kernel-version floor gate.
- remove: drop _ntsync_state kernel-version branch.
- docs: README drops kernel badge/row/notes.

7.17.29  2026-06-01
- comment: reword <6.14 floor comment to "non-aborting".
- docs: uninstall note — PKGS_ADD includes Vulkan/gaming deps.

7.17.28  2026-06-01
- comment: condense over-length/multi-line comments to single lines (<= 120 cols).

7.17.27  2026-06-01
- fix: --check keeps confirmed drift as EXIT_DRIFT (10), not EXIT_PREFLIGHT (3).
...
- change: fstab ext4 rewrite tab-separates rewritten lines; idempotent.

7.17.23  2026-05-31
- fix: --install-file modules-load.d/* modprobes immediately; hooks 16 -> 17.
...
7.17.21  2026-05-31
- feat: pin NVMe scheduler to none (60-ry-ioschedulers.rules); 15 -> 16, hooks 15 -> 16.
...
7.17.19  2026-05-31
- fix: _tmpfile_key uses literal HOME-prefix compare.
- refactor: extract _vrsv_wifi_nm_backend.
- docs: exit-code table notes code 1 covers general install FAIL.

7.17.18  2026-05-31
- fix: route modules-load.d/i2c-dev.conf to bare-module validator (regression from 7.17.16).
- docs: content-generator count 13 -> 15.

7.17.17  2026-05-31
- change: wireless regdom mandatory (default US); 14 -> 15.
...
7.17.16  2026-05-31
- feat: add ddcutil, rtkit; ship i2c-dev.conf; 13 -> 14, PKGS_ADD 13 -> 15.
- feat: opt-in --country=XX regdom (advisory).
...
7.17.14  2026-05-31
- fix: paru-absent advisory (WARN + continue).
...
- fix: PKGS_DEL records actual removed count; db.lck during removal FAIL.
- docs: verdict table gains Exit column + two preflight-bypass exits.

7.17.13  2026-05-31
- harden: _acquire_lock stale-reclaim retries 3x.
- fix: _awf_postwrite_verify_restore logs WARN when re-verify skipped.
- docs: Prerequisites adds kernel-version map.
- docs: --help notes -h/-v honored before all checks.
...
- cleanup: _dc_erase_globals erases fstab/sysctl globals.

7.17.11  2026-05-31
- fix: force-print boot-critical DO NOT REBOOT banner in QUIET install.
- fix: verify sudo-cache bail symmetric across static/runtime arms.
- fix: drop stray JSONL-only _phase_record in _vrsv_wifi.
- harden: write .ry.bak only after render + symlink-probe.

7.17.10  2026-05-31
- fix: add ry-tee-err.* to fs-sweep globs.
- comment: condense three rationale comments.

7.17.9  2026-05-31
- fix: count installed-bytes collect failure once in _verify_static_checksum.
- harden: systemd >= 250 a true hard gate.
- harden: allocate _run overflow-spill via mktemp --suffix=.log.

7.17.8  2026-05-31
- fix: export HOME via set -gx on getent-recovery path.
- fix: signal-time lock cleanup removes dir only when held by us.
- harden: _run timeout-bypass skips env and VAR=val tokens after sudo.

7.17.7  2026-05-31
- format: split --INSTALL-FILE banner into DISPATCH TABLE + POST-HOOK HANDLERS.

7.17.6  2026-05-31
- format: split VERIFY-STATIC SYSTEM into SYSTEM+USER / PACKAGES+SERVICES+SYNTAX / CHECKSUM+DRIVER.

7.17.5  2026-05-31
- format: split VERIFY-RUNTIME banner into SERVICES / ENVIRONMENT / SESSION+PERMS.

7.17.4  2026-05-31
- kernel: condense _ry_check_kernel_version to <6.14 hard-floor.
- cleanup: remove unused RC_KVER_WARN code.
...
- verify: split drirc xmllint check into _vrs_drirc_xml.
...
- verify: assert masked units inactive, NM backend==iwd, drirc/modprobe.
...
7.16.4  2026-05-30
- docs: tighten Prerequisites sudo-cache warning.
...
7.13.4  2026-05-29
- comment: condense comments to single-line.
