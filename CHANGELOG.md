...
v3.51.11  2026-04-16
- Sourcing detection (HIGH): replaced `status is-interactive` with `status stack-trace 2>/dev/null | string match -q '*from sourcing*'` at lines 5 and 15. Previous gate correctly detected interactive sourcing (`source ry-install.fish --version` from a live shell) but failed for non-interactive sourcing (`fish -c 'source ry-install.fish --version'`). Root cause: fish's `exit` inside a sourced file acts as return-from-source regardless of `status is-interactive`, so the `exit $code` path in `_ry_exit` returned to the caller without running `_ry_namespace_cleanup`, leaking 45 globals (`VERSION`, `EXIT_*`, `LOG_*`, `KVER*`, `PROGRESS_*`, `VERIFY_*`, etc.) into the host shell. `status stack-trace` reliably distinguishes sourced-vs-direct in both interactive and non-interactive contexts (fish 3.4+): output contains `from sourcing file` when sourced, empty when run directly. The same fix was applied to the re-source guard at line 5 which previously used `status is-interactive` for the `return`-vs-`exit` decision.
- `_ry_exit` defense-in-depth (MED): namespace cleanup now runs unconditionally before both the `return` and `exit` paths. Previously cleanup only ran on the `_RY_INSTALL_SOURCED=true` branch. When running directly, `exit` kills the process so leaked globals are moot, but the unconditional cleanup closes the edge case where fish's `exit`-as-return-from-source behavior could leak globals if the sourcing detection ever regresses.
...
- Audit: `fish --no-execute` passes. Non-interactive source leaks 2 globals (intentional caller-API: `_RY_INSTALL_LAST_EXIT`, `_RY_INSTALL_BAILING`). Interactive source leaks 2 globals (same). Direct execution exits cleanly. Re-source guard functional. All modes verified: `--version`, `--help`, `--completions`, `--check`, `--bogus`, `--verify-static`, combined flags, `--install-file` without path, deprecated `--dry-run`/`--all`.

v3.51.9  2026-04-15
- `_acquire_lock` log PID literals (LOW): `_log "LOCK_ACQUIRED: pid=%self dir=$LOCK_DIR"` at line 377 and `_log "LOCK_RECLAIMED: stale pid=$old_pid, new pid=%self"` at line 436 embedded `%self` inside double-quoted strings. Fish's process-spec expansion (`%self` → fish PID) only fires on bareword arguments — inside double quotes it is treated as literal text, so both log entries recorded the string `pid=%self` instead of the actual PID. Diagnostic impact: lock-ownership debugging and reclaim-audit queries against `{"event":"lock_acquired"}` / `{"event":"lock_reclaimed"}` JSONL records would show an uninterpretable PID value. Fix: replace `%self` → `$fish_pid` (fish 3.1+ documented variable; expands inside quotes as expected). Both sites verified via interactive source + lock-acquire trace.
- `%self` → `$fish_pid` consistency (LOW): 6 additional `%self` bareword uses (`_acquire_lock` at lines 371, 397, 419, 424, 466; `_sudo_keepalive` at line 4708) functionally correct but stylistically inconsistent with the `$fish_pid` use at line 99 (TIMESTAMP suffix). Standardized on `$fish_pid` across all 8 sites — single canonical form, matches fish documentation preference, eliminates the "expands as bareword / doesn't expand in quotes" footgun that caused the two log bugs above.
- `command install -m 0600 /dev/null "$LOG_FILE"` at lines 134 and 6112 (INFO): added `--` end-of-options separator. Both paths (`/dev/null` literal and `$HOME`-rooted log path) cannot begin with `-` in practice, so this is hygiene only — closes the gap with the otherwise-consistent `--` discipline across all other `rm`/`mv`/`cp`/`chmod`/`chown` sites in the script (510 `--` uses).
- No profile changes, no new dependencies, no managed-file count change (still 16), no kernel parameter changes, no embedded-config hash drift. All changes are in lock-acquire/reclaim logging and log-file initialization.
- README.md: version banner and sample log version bumped to v3.51.9.
- Audit: `fish --no-execute` passes; interactive smoke test confirms no regression in namespace cleanup, host-global preservation, or re-source enablement.

v3.51.8  2026-04-15
- Namespace cleanup on source-bail and source-completion (MED): sourced runs now erase every `set -g` global the script created, preserving host-shell namespace. Previous behavior leaked 174 globals (`VERSION`, `LOG_FILE`, `EXIT_*`, `KVER*`, `PROGRESS_*`, `VERIFY_*`, etc.) into the caller's fish session, and `_RY_INSTALL_LOADED` was never erased — permanently blocking re-source in the same shell until manual `set -e _RY_INSTALL_LOADED`. New mechanism: `_RY_PRE_GLOBALS` snapshots `set --names -g` before the first `set -g` (line 14); `_ry_namespace_cleanup` iterates current globals and erases any not in the snapshot, except `_RY_INSTALL_LAST_EXIT` (caller reads result) and `_RY_INSTALL_BAILING` (bail-path only, for in-flight sentinel propagation). Called from `_ry_exit` source-bail branch and end-of-file source-completion branch. Critical implementation detail: `_RY_PRE_GLOBALS` is itself a post-snapshot global, so the erase loop copies it to `set -l _snap` before iterating — without this, erasing `_RY_PRE_GLOBALS` mid-loop caused subsequent `contains` checks to fail and wiped host-shell `__fish_*` internals. Verified: interactive source + bail erases all script globals, preserves all host globals, re-source proceeds past loaded-guard.
- Redundant `2>/dev/null` removed from 4 `set --erase` call sites (LOW): `set --erase` is a no-op on missing variable names in fish 3.4+; the redirect suppressed nothing. Affected sites: `_RY_SKIP_IWD` and `_RY_SKIP_IWD_CACHED` in `_do_cleanup` and `_install_post_package_refresh`.
- No profile changes, no new dependencies, no managed-file count change (still 16), no kernel parameter changes, no embedded-config hash drift. All changes are in the source-bail/completion path. Script structure unchanged except for the `_ry_namespace_cleanup` helper and its two call sites.
- README.md: version banner and sample log version bumped to v3.51.8.
- Audit: `fish --no-execute` passes; interactive smoke test confirms namespace cleanup on bail, host-global preservation, and re-source enablement.
